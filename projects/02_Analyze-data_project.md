...
```{code-cell} ipython3
ebay_data = data[data['symbol'] == 'EBAY']

plt.plot(ebay_data['date'], ebay_data['open'])

# remove data > 2015-07-20

ebay_data_pre_2015 = ebay_data[ ebay_data['date'] < pd.to_datetime('2015-07-20')]
plt.plot(ebay_data_pre_2015['date'], ebay_data_pre_2015['open'])
plt.xlabel('date')
plt.ylabel('opening price (\$)');
```

```{code-cell} ipython3
dprice = np.diff(ebay_data_pre_2015['open'])
plt.plot(ebay_data_pre_2015['date'][1:], dprice)
plt.xlabel('date')
plt.ylabel('change in opening price (\$/day)');
```
...
plt.hist(dprice, 50, density=True)
plt.plot(x, price_pdf)
plt.title('EBAY changes in price over 5 years\n'+
         'avg: \${:.2f} stdev: \${:.2f}'.format(mean_dprice, std_dprice));
```
...
rng = default_rng(42)
N_models = 100
dprice_model = rng.normal(size = (len(ebay_data_pre_2015), N_models), loc = 0.03038, scale = 0.7228)
...
```{code-cell} ipython3
price_model = np.cumsum(dprice_model, axis = 0) + ebay_data_pre_2015['open'].values[0]

plt.plot(ebay_data_pre_2015['date'], price_model, alpha = 0.3);

plt.plot(ebay_data_pre_2015['date'], ebay_data_pre_2015['open'], c = 'k', label = 'NYSE data')
plt.xlabel('date')
plt.ylabel('opening price (\$)');
```
...
skip = 100
plt.errorbar(ebay_data_pre_2015['date'][::skip], price_model_avg[::skip],
             yerr = price_model_std[::skip], 
             fmt = 'o',
             c = 'r', 
             label = 'model result', 
